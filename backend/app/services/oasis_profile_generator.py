...
class OasisProfileGenerator:
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if entity.related_nodes:
            ...
            if related_info:
                context_parts.append("### Information on connected entities\n" + "\n".join(related_info))
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are an expert at generating social-media user personas. Produce detailed, realistic personas for an opinion simulation, staying as close as possible to the real situation described. You must return valid JSON, and no string value may contain an unescaped newline."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media user persona for this entity, staying as close as possible to the real situation described.
...
2. persona: detailed persona description (around 1200 words of plain text), covering:
   ...
   - Positions and views (attitude to the topic, what would anger or move them)
   - Distinctive traits (catchphrases, unusual experiences, hobbies)
   - Personal memory (an important part of the persona: how this individual relates to the event, and what they have already said or done in it)
...
Important:
- Every field value must be a string or a number, with no newlines
- persona must be one continuous piece of prose
- {get_language_instruction()} (the gender field must still be the English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social-media account profile for this organisation or group entity, staying as close as possible to the real situation described.
...
2. persona: detailed account description (around 1200 words of plain text), covering:
   ...
   - Institutional memory (an important part of the persona: how this organisation relates to the event, and what it has already said or done in it)
3. age: always 30 (the notional age of an institutional account)
4. gender: always "other" (institutional accounts use other, meaning non-individual)
5. mbti: MBTI type, used to characterise the account's voice, e.g. ISTJ for rigorous and conservative
6. country: country name
...
- persona must be one continuous piece of prose with no newlines
