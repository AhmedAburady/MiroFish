...
class SimulationRunner:
    ...
    def forget_simulation(cls, simulation_id: str, join_timeout: float = 5.0):
        """让 Runner 忘记一个模拟。删除目录前必须先调用且必须成功。

        监控线程会调用 _save_run_state()，那是 makedirs(exist_ok=True) 加写文件，
        会把刚删掉的目录重建出来。因此线程没退出就绝不能删目录：此时保留线程
        注册，返回 ok=False，让调用方拒绝本次删除并允许稍后重试。

        Returns:
            (ok, warnings)  warnings 是 [{'code': str, ...params}]，
            由 API 层本地化，Runner 不产出面向用户的文案。
        """
        warnings = []

        thread = cls._monitor_threads.get(simulation_id)
        if thread and thread.is_alive():
            thread.join(timeout=join_timeout)
            if thread.is_alive():
                # 注意：线程仍留在 _monitor_threads 里，重试时可以再次 join
                return False, [{'code': 'monitor_alive',
                                'id': simulation_id,
                                'timeout': join_timeout}]
        cls._monitor_threads.pop(simulation_id, None)

        for stream, handles in (('stdout', cls._stdout_files), ('stderr', cls._stderr_files)):
            fh = handles.pop(simulation_id, None)
            if fh:
                try:
                    fh.close()
                except Exception as e:
                    warnings.append({'code': 'handle_close_failed',
